...
**Gofer** — это консольный клиент-серверный мессенджер.
...
- **Интерфейс:** [**`Bubble Tea`**](https://github.com/charmbracelet/bubbletea) — TUI интерфейс
- **Архитектура:** Clean Architecture, 4 слоя

Два бинарника: **сервер** (HTTP + WebSocket + Postgres) и **TUI-клиент**.
...
- **Поиск пользователей** — по ID пользователя.
- **Выбор сервера в клиенте** — список серверов хранится локaльно, переключение и проверка доступности (`●` онлайн / офлайн) прямо из TUI, без пересборки.
